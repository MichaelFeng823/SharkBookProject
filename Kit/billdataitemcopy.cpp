#include "billdataitemcopy.h"
#include <QEvent>
#include "Util/clog.h"

BillDataItemCopy::BillDataItemCopy(InAndOutType type,QWidget *parent) : QWidget(parent)
{
    LOG("BillDataItem()");
    currenttype = type;
    buildUiLayout();
...
//构建界面布局
void BillDataItemCopy::buildUiLayout()
{
    //构建Title部分布局
...
//构建界面样式
void BillDataItemCopy::buildUiStyle()
{
    //设置Title部分样式
...
void BillDataItemCopy::setDate(QDate date)
{
...
void BillDataItemCopy::setTitlePayNum(double num)
{
...
void BillDataItemCopy::setPayType(QString strType)
{
    strPayType = strType;
    labelPayType->setText(strPayType);
}
void BillDataItemCopy::setIcon(QString strIcon)
{
...
void BillDataItemCopy::setDetialPayNum(double num)
{
...
void BillDataItemCopy::setPayDetailSoucres(QString strSources)
{
    strPayDetailSources = strSources;
    labelPayDetailSources->setText(strPayDetailSources);
}

bool BillDataItemCopy::eventFilter(QObject *obj, QEvent *event)  //事件过滤器
{
    if(obj == titleWidget)
    {
