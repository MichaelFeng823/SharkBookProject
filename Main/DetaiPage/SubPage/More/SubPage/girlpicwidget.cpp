#include "girlpicwidget.h"
#include "ui_girlpicwidget.h"
#include <QImage>
#include <QPixmap>
#include <QPicture>
#include <QPointer>
...
int GirlPicWidget::m_DefaultWidth = 1080;
GirlPicWidget::GirlPicWidget(QWidget *parent) :
    BaseCustomWidget(parent),
    ui(new Ui::GirlPicWidget)
{
    ui->setupUi(this);
    ...
    connect(m_AtuoUpdatePicTimer,&QTimer::timeout,this,&GirlPicWidget::checkNetWorkOnline);
    connect(MSGQUEUE, SIGNAL(SignalRecvMsg(QByteArray,QObject *)), this, SLOT(onRecvMsg(QByteArray, QObject *)));
    connect(photo,&PhotoFrame::requestClose,this,[=](){
        this->hide();
        this->setActiveWindow(false);

    });
}
GirlPicWidget::~GirlPicWidget()
{
    delete ui;
}
//设置当前Active状态
void GirlPicWidget::setActiveWindow(bool state)
{
...
//自动定时更新槽函数
void GirlPicWidget::onAutoUpdateSlot()
{
    if(isActiveWindow()){
...
//当收到消息时的槽函数
void GirlPicWidget::onRecvMsg(QByteArray data, QObject * obj)
{
    if(obj != this)
        return;
    //请求返回的结果
...
//检查收到数据内容
DataType GirlPicWidget::checkDataType(QByteArray data)
{
...
void GirlPicWidget::checkNetWorkOnline()
{
    QHostInfo::lookupHost("www.baidu.com",this,SLOT(onLookupHost(QHostInfo)));
}

void GirlPicWidget::onLookupHost(QHostInfo host)
{
